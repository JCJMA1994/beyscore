...
class _ComboBuilderPageState extends State<ComboBuilderPage> {
  ...
  Future<void> _openMetaPresetsModal() async {
    const presetService = MetaPresetService();
    final presets = presetService.getPresets();
    final blades = await _catalogRepo.watchByType(PartType.blade).first;
    final ratchets = await _catalogRepo.watchByType(PartType.ratchet).first;
    final bits = await _catalogRepo.watchByType(PartType.bit).first;
    final allParts = <Part>[...blades, ...ratchets, ...bits];
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Icon(Icons.auto_awesome, color: AppColors.x, size: 20),
                const SizedBox(width: 8),
                Text(
                  'COMBOS META COMPETITIVOS (WBO 2026)',
            ...
            Flexible(
              child: ListView.separated(
                ...
                itemBuilder: (context, idx) {
                  final p = presets[idx];
                  final color = switch (p.archetype) {
                  ...
                  return InkWell(
                    onTap: () {
                      final blade = allParts.firstWhere(
                        (Part part) => part.id.toLowerCase() == p.bladeId.toLowerCase() || part.name.toLowerCase().contains(p.bladeId.replaceAll('_', ' ')),
                        orElse: () => allParts.firstWhere((Part part) => part.type == PartType.blade),
                      );
                      final ratchet = allParts.firstWhere(
                        (Part part) => part.id.toLowerCase() == p.ratchetId.toLowerCase() || part.name.toLowerCase() == p.ratchetId.toLowerCase(),
                        orElse: () => allParts.firstWhere((Part part) => part.type == PartType.ratchet),
                      );
                      final bit = allParts.firstWhere(
                        (Part part) => part.id.toLowerCase() == p.bitId.toLowerCase() || (part.code != null && part.code!.toLowerCase() == p.bitId.toLowerCase()),
                        orElse: () => allParts.firstWhere((Part part) => part.type == PartType.bit),
                      );

                      setState(() {
                        _selectedSystem = p.system;
                        _selectedBlade = blade;
                        _selectedRatchet = ratchet;
                        _selectedBit = bit;
                      ...
                      child: Row(
                        children: [
                          Container(
                            ...
                            child: Text(
                              'TIER ${p.tier}',
                              style: AppTypography.mono.copyWith(fontSize: 10, fontWeight: FontWeight.bold, color: color),
                            ),
                          ),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  '${p.archetype.toUpperCase()} · ${p.description}',
                          ...
                          const Icon(Icons.arrow_forward_ios, size: 14, color: AppColors.mute),
                        ],
                      ),
                    ),
                  );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton.icon(
            icon: const Icon(Icons.auto_awesome, size: 16, color: AppColors.x),
            label: Text(
              'META PRESETS',
              style: AppTypography.mono.copyWith(fontSize: 11, fontWeight: FontWeight.bold, color: AppColors.x),
            ),
            onPressed: _openMetaPresetsModal,
          ),
        ],
      ),
